...
class InteractiveBrokersExecutionClient(LiveExecutionClient):
    ...
    async def generate_position_status_reports(
        ...
        # TODO: How to add the positions from on startup? Using the previous ClientOrderId.
        # Cannot reconcile position: EUR/USD.IDEALPRO net qty 0.0 != reported -100000.0.

        report = []
        positions = await self._client.get_positions(self.account_id.get_id())
        for contract_id in positions.keys():
            report.append(
                self._parse_position(
                    instrument_id=self.instrument_provider.contract_id_to_instrument_id[
                        contract_id
                    ],
                    position=positions[contract_id]["position"],
                    avg_cost=positions[contract_id]["avg_cost"],
                ),
            )
        return report

    def _parse_position(self, instrument_id: InstrumentId, position: Decimal, avg_cost: float):
        if position > 0:
        ...
        return PositionStatusReport(
            ...
            instrument_id=instrument_id,
            position_side=side,
            quantity=Quantity.from_int(abs(position)),
            report_id=UUID4(),
